...
def main(directory, catalog_path, prefer_public=False):
    ...
    for xsd_file in xsd_files:
        ...
        if not schema_ns:
            ...
            continue
        rel_path = os.path.relpath(xsd_file, ".")
        # Create a <public> entry
        catalog_entries.append(f'    <public publicId="{schema_ns}" uri="{rel_path}"/>')
    ...
    for dtd_file in dtd_files:
        ...
        catalog_entries.append(f'    <public publicId="{public_id}" uri="{rel_path}"/>')
        # Create a <system> entry for SYSTEM identifier
        catalog_entries.append(f'    <system systemId="{system_id}" uri="{rel_path}"/>')
    ...
    rewrite_prefix = os.path.abspath(".")
    if common_prefix:
        catalog.append(
...
Examples:
  ...
  %(prog)s test/dtd slee --prefer-public
    ...
    parser.add_argument("directory", help="Directory containing XSD or DTD files")
    parser.add_argument(
        "catalog_prefix",
